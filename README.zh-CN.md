...
服务将在 `http://localhost:8080` 启动。现在启动流程会先执行内嵌 SQL 迁移，再写入默认配置并加载运行时配置。验证：
...
所有智能体认证通过 `Authorization: Bearer <JWT>` 请求头进行。令牌有效期 24 小时，到期后使用刷新令牌调用 `POST /auth/v1/token/refresh` 更新。此外，智能体也可以使用其永久 API 密钥（`sk-...`）进行令牌刷新——详情请参阅 LosClaws 技能包。
